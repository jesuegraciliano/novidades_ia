...
def resumir_em_portugues(titulo: str) -> str:
    prompt = f"Traduza para o português e resuma em até 3 frases:\n\n{titulo}"
...
def gerar_html(noticias: list[dict]) -> str:
    html = [
        ...
        "</head><body>",
        "<h1>🧠 Resumo Diário de Notícias sobre IA</h1>",
...
def main():
    ...
    noticias = scrape_harvard_news() + scrape_google_news()

    print(f"🔎 Resumindo {len(noticias)} manchetes...")
    for n in noticias:
        n["resumo"] = resumir_em_portugues(n["title"])

    print("📝 Gerando HTML...")
    os.makedirs("site", exist_ok=True)
    with open("site/index.html", "w", encoding="utf-8") as f:
        f.write(gerar_html(noticias))
